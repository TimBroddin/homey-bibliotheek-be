...
class BibliotheekAPI {
  ...
  async getLoanDetails(url) {
    ...
    libs.each((_, libDiv) => {
      ...
      books.each((_, bookEl) => {
        ...
        // Extract extend_loan_id from checkbox and check extendability
        let extendLoanId = '';
        let extendWarning = '';
        const extendDiv = $book.find('.my-library-user-library-account-loans__extend-loan');

        try {
          const checkbox = extendDiv.find('input[type="checkbox"]');
          extendLoanId = checkbox.attr('id') || '';
        } catch { /* ignore */ }

        // If no checkbox, check for warning message
        if (!extendLoanId) {
          const warningDiv = extendDiv.find('.card--warning');
          if (warningDiv.length) {
            extendWarning = warningDiv.text().trim() || warningDiv.attr('data-tippy-content') || '';
          }
        }
   ...
   * Based on the Python implementation from myTselection/bibliotheek_be
   * The extension now works by simply making GET requests to the extend URL
  ...
  async extendLoans(baseUrl, extendLoanIds) {
    ...
    // Build extension URL with comma-separated loan IDs (URL encoded)
    const extendUrl = `${baseUrl}/extend?loan-ids=${extendLoanIds.join('%2C')}`;

    this.log(`Extending loans: ${extendUrl}`);

    // Make GET request to the extension URL - this triggers the extension
    ...
    this.log(`Extension response status: ${response.status}`);

    // Also fetch the base URL to confirm/complete the extension
    const baseResponse = await this.fetch(baseUrl, {
      headers: this.baseHeaders,
      redirect: 'follow',
      timeout: TIMEOUT
    });

    this.log(`Base URL response status: ${baseResponse.status}`);
